...
puts ""
puts "1 --> Combien y-a t'il de journaliste dans cet array ?"
puts "Il y a #{array.count} journalistes dans cet array."

#Combien d'handle contiennent un numéro ?
puts ""
puts "2 --> Combien d'handles contiennent un numero ?"

count = 0               #def variable d'incrémentation
array.each do |handle|           #each do = pour chaque element du tab
  if handle.match(/[0-9]/)           #match= matcher [correspond]
  count +=1
  end
end
...
puts "Il y a #{count} handle qui contiennent les 4 lettres."
...
puts "Il y a au total, #{count} underscore."
...
puts array.sort_by { |word| word.length }.first(50)
...
puts "La place de @epenser dans l'array est #{array.find_index("@epenser")}."
